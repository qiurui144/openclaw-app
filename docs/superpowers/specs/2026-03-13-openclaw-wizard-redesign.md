...
9. HTTP 健康检查（轮询 `http://127.0.0.1:{port}/health`，最多 30s，间隔 2s）
   - 通过 → 继续
   - 超时前先检查端口占用：若端口被**非 openclaw 进程**占用，立即中止轮询，错误消息标注"端口 {port} 已被进程 {pid}/{name} 占用，请更换端口后重试"
   - 超时（服务崩溃/启动慢）→ 展示"服务未能正常启动"，给出两个选项："查看日志"/ "忽略并继续"
...
- macOS 向导静默自动更新：macOS 仅提示"有新版本"，用户需手动下载新包重新运行，当前已安装的 openclaw 服务不受影响
...
注意：`SecretString`（`secrecy::Secret<String>`）未实现 `Serialize`，`DeployConfig` 的 IPC 传输需要一个专用的 DTO 类型（去掉密码字段），在 `main.rs` 的 command handler 中手动合并密码。
