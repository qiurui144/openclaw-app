...
9. HTTP 健康检查（轮询 `http://127.0.0.1:{port}/health`，最多 30s，间隔 2s）
   - 通过 → 继续
   - 超时 → 展示"服务未能正常启动"错误，给出两个选项："查看日志"（展开日志区）/ "忽略并继续"（进入 FinishPage 但显示警告横幅）
...
- 大号成功图标 + 完成提示
- "🌐 打开管理控制台"（1秒后自动打开浏览器）
- **安装信息摘要**（一键复制）：服务地址、管理员密码提示、配置文件路径（`~/.openclaw/openclaw.json`，告知用户忘记密码可在此查看）
...
pub struct DeployConfig {
    ...
    pub source_mode: SourceMode,    // Bundled | Online | LocalZip(path)
...
5. 解压 .tgz 到临时目录（tar xzf，Rust 内部实现，不依赖系统 tar 命令）
6. 将解压内容 mv 到 node_modules/@openclaw/{skill-name}/（原子替换）
7. 发送 SIGHUP 到 openclaw 进程（热重载，无需重启）
```
...
| 字段 | 值 |
|---|---|
| 项目 | Mihomo（MetaCuban 维护） |
| 仓库 | `https://github.com/MetaCuban/mihomo` |
| 许可证 | GPL-3.0 |
